...
fn run() -> Result<(), failure::Error> {
    ...
    let window = video_subsystem
        .window("Yarge | Declarative UI", WIDTH, HEIGHT)
        .opengl()
        .resizable()
        .build()
        .unwrap();
    ...
    let my_view_text = primitives::Text::new(primitives::TextProps {
        text: "A Child ViewText with font".to_string(),
        settings: font::TextSettings {
            font: "dejavu".to_string(),
            width: viewport.w as f32 / 2.0,
            size: 18.0.into(),
            pos: (200.0, 200.0),
    ...
    let my_child_view = View {
        ...
        children: vec![Box::new(my_view_text)],
    };
    let my_view = View {
        props: ViewProps {
            width: 400.0,
        ...
        children: vec![Box::new(my_child_view)],
    };

    let my_view_props = my_view.props();

    println!("MYVIEWCOMP {:#?}", my_view);
    println!("MyView name {:#?}, id {:#?}, render {:#?}, children {:#?}",
        my_view.name(),
        my_view.id(),
        my_view.render(),
        my_view.children()
    );
    println!("MYVIEWPROPS {:#?}", my_view_props);
